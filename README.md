# Zero‑Cloud Personal AI Agents: Sovereign, Private Video, and Claude‑Code‑CLI + Ollama – A Privacy‑First Study
...
- **SovereignAgent**  
  ...
  - Uses Gemma‑431B for reasoning, LLaVA/qwen3‑vl for vision, and Whisper for audio transcription.  
  - Generates a concise, HTML‑styled briefing and pushes it to a private Telegram bot, guaranteeing 100 % local privacy (no data leaves the computer).

- **PrivateVideoAgent**  
  - Extracts key frames from video files, transcribes audio with Whisper, and performs deterministic “needle‑in‑a‑haystack” searches across large video collections.  
  - Leverages Gemma‑431B (or Gemma‑4) for multimodal frame and transcript analysis, with a 256 K‑token context window that keeps full transcripts and visual cues in memory.  
  - Operates on an air‑gapped workstation, ensuring zero data leakage while enabling fast, accurate video content discovery.

- **ClaudeCodeCLI + Ollama Agent**  
