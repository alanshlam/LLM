...
def get_latest_emails(filepath, limit=5):
    """Fetches emails and returns (AI Context String, Display String)"""
    ...
    try:
        ...
        # Grab the latest `limit` messages
        ...
        for msg in latest_msgs:
            ...
            # Extract a snippet of the email body for the AI to analyze
            body_snippet = ""
            if msg.is_multipart():
                for part in msg.walk():
                    if part.get_content_type() == 'text/plain':
                        try:
                            ...
                            body_snippet = " ".join(body.split())[:150] # Get first 150 chars
                            break
                        except: pass
            else:
                try:
...
def generate_private_briefing(schedule, tasks, expenses, weather, emails, model):
    """Uses LLM to act as a reasoning engine over private data including emails."""
...
def main():
    ...
    # We now get both the raw text string (for AI context) and formatted HTML (for display)
