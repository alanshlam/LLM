...
def get_latest_emails(filepath, limit=2):
    if not os.path.exists(filepath): return "<i>No mailbox found.</i>"
    try:
        mbox = mailbox.mbox(filepath)
        messages = list(mbox)
        if not messages: return "<i>Inbox is empty.</i>"
        ...
        for msg in latest_msgs:
            ...
            lines.append(f"▪️ <b>{display}</b>: {subject}")
        return "\n".join(lines)
    except: return "<i>Error reading mailbox.</i>"
...
def fetch_category_news(category, query, max_results=5):
    for attempt in range(3):
        try:
            print(f"🔍 Searching {category} (Attempt {attempt+1})...")
            results = list(DDGS().news(query=query, region="wt-wt", safesearch="moderate", timelimit="d", max_results=max_results))
...
def generate_private_briefing(schedule, tasks, expenses, weather, model):
    """Uses LLM to act as a reasoning engine over private data."""
    ...
    INSTRUCTIONS: Provide exactly 3 short bullet points. Do not use bold tags, just emojis.
...
def generate_news_briefing(ai_news, cyber_news, audio_text, audio_url, model):
    ...
    try:
        ...
        # Inject formatted Audio Link directly under the generated Audio section header
        if audio_url:
...
def main():
    ...
    emails = get_latest_emails(MBOX_PATH)

    # 3. Gather News
    audio_text, audio_url = process_audio_news()
    
    # Increased max_results to 5 to give the LLM more options to choose 3 from
    ai_news = fetch_category_news("AI", "artificial intelligence breakthroughs", 5)
    print("⏳ Cooling down 10s...")
    time.sleep(10)
    cyber_news = fetch_category_news("Cybersecurity", "cybersecurity vulnerabilities", 5)

    # 4. LLM Generation
    private_briefing = generate_private_briefing(schedule, tasks, expenses, weather, OLLAMA_SUMMARY_MODEL)
